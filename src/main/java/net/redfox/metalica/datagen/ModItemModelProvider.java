...
public class ModItemModelProvider extends MyItemModelProvider {
  ...
  private MyItemModelBuilder simpleItem(Item item, String modelName) {
    String itemName =
        item.getDescriptionId()
            .substring(
                item.getDescriptionId().indexOf(Metalica.MOD_ID) + 1 + Metalica.MOD_ID.length());
    return withExistingParent(
            Metalica.MOD_ID + ":item/" + itemName, ResourceLocation.parse("item/generated"))
        .texture("layer0", Metalica.MOD_ID + ":item/" + modelName);
  }
  ...
  private MyItemModelBuilder bucketItem(Item item, String modelName) {
    ...
    return withExistingParent(
        ...
        .texture("layer1", Metalica.MOD_ID + ":item/" + modelName);
  }
}
