...
public class ModBlockTagProvider extends BlockTagsProvider {
  ...
  protected void addTags(HolderLookup.Provider pProvider) {
    for (MetalMaterial material : MetalMaterial.getMaterials()) {
      this.tag(material.getStorageBlockMiningLevel()).add(material.getStorageBlock().get());
      this.tag(BlockTags.MINEABLE_WITH_PICKAXE).add(material.getStorageBlock().get());
