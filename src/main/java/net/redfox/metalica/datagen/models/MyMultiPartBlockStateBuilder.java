...
public final class MyMultiPartBlockStateBuilder implements IGeneratedBlockState {
  ...
  /**
   * Creates a builder for models to assign to a {@link MyMultiPartBlockStateBuilder.PartBuilder},
   * which when completed via {@link MyConfiguredModel.Builder#addModel()} will assign the resultant
   * set of models to the part and return it for further processing.
   *
  ...
  public class PartBuilder {
    ...
    public final List<MyMultiPartBlockStateBuilder.PartBuilder.ConditionGroup>
        nestedConditionGroups = new ArrayList<>();
    ...
    /**
     * Makes this part get applied if any of the conditions/condition groups are true, instead of
     * all of them needing to be true.
     */
    ...
    /**
     * Set a condition for this part, which consists of a property and a set of valid values. Can be
     * called multiple times for multiple different properties.
     *
     ...
     * @throws IllegalArgumentException if {@code prop} is not applicable to the current block's
     *     state
     * @throws IllegalStateException if {@code !nestedConditionGroups.isEmpty()}
     */
    @SafeVarargs
    public final <T extends Comparable<T>> MyMultiPartBlockStateBuilder.PartBuilder condition(
        Property<T> prop, T... values) {
      ...
      Preconditions.checkArgument(
          ...
          prop.getName());
      Preconditions.checkArgument(
          canApplyTo(owner), "IProperty %s is not valid for the block %s", prop, owner);
    ...
    public final MyMultiPartBlockStateBuilder.PartBuilder.ConditionGroup nestedGroup() {
      ...
      MyMultiPartBlockStateBuilder.PartBuilder.ConditionGroup group =
          new MyMultiPartBlockStateBuilder.PartBuilder.ConditionGroup();
    ...
    JsonObject toJson() {
      ...
      if (!conditions.isEmpty()) {
        ...
        out.add(
            "when", MyMultiPartBlockStateBuilder.toJson(this.nestedConditionGroups, this.useOr));
      }
    ...
    public class ConditionGroup {
      ...
      /**
       * Set a condition for this part, which consists of a property and a set of valid values. Can
       * be called multiple times for multiple different properties.
       *
       * @param <T> the type of the property value
       * @param prop the property
       * @param values a set of valid values
       ...
       */
      @SafeVarargs
      public final <T extends Comparable<T>>
          MyMultiPartBlockStateBuilder.PartBuilder.ConditionGroup condition(
              Property<T> prop, T... values) {
        Preconditions.checkNotNull(prop, "Property must not be null");
        Preconditions.checkNotNull(values, "Value list must not be null");
        Preconditions.checkArgument(values.length > 0, "Value list must not be empty");
      ...
      public MyMultiPartBlockStateBuilder.PartBuilder end() {
        if (this.parent != null)
          throw new IllegalStateException(
              "This is a nested condition group, use endNestedGroup() instead");
  ...
  private static JsonObject toJson(
      List<MyMultiPartBlockStateBuilder.PartBuilder.ConditionGroup> conditions, boolean useOr) {
  ...
  private static JsonObject toJson(Multimap<Property<?>, Comparable<?>> conditions, boolean useOr) {
    ...
    for (Map.Entry<Property<?>, Collection<Comparable<?>>> e : conditions.asMap().entrySet()) {
      ...
      for (Comparable<?> val : e.getValue()) {
        if (activeString.length() > 0) activeString.append("|");
