...
public final class MyConfiguredModel {
  ...
  public static MyConfiguredModel[] allYRotations(
      ModelFile model, int x, boolean uvlock, int weight) {
   ...
   * @throws IllegalArgumentException if x and/or y rotation are not valid (see {@link
   *     BlockModelRotation})
   * @throws IllegalArgumentException if weight is less than or equal to zero
   */
  public MyConfiguredModel(
      ModelFile model, int rotationX, int rotationY, boolean uvLock, int weight) {
  ...
  /**
   * Construct a new {@link MyConfiguredModel} with the {@link #DEFAULT_WEIGHT default random
   * weight}.
   *
   * @param model the underlying model
   * @param rotationX x-rotation to apply to the model
  ...
  /**
   * Construct a new {@link MyConfiguredModel} with the default rotation (0, 0), uvlock (false), and
  ...
  static void checkWeight(int weight) {
    Preconditions.checkArgument(
        weight >= 1, "Model weight must be greater than or equal to 1. Found: %d", weight);
  }
  ...
  static MyConfiguredModel.Builder<MyVariantBlockStateBuilder> builder(
      MyVariantBlockStateBuilder outer, MyVariantBlockStateBuilder.PartialBlockstate state) {
    return new MyConfiguredModel.Builder<>(
        models -> outer.setModels(state, models), ImmutableList.of());
  }

  static MyConfiguredModel.Builder<MyMultiPartBlockStateBuilder.PartBuilder> builder(
      MyMultiPartBlockStateBuilder outer) {
    return new MyConfiguredModel.Builder<MyMultiPartBlockStateBuilder.PartBuilder>(
        models -> {
          MyMultiPartBlockStateBuilder.PartBuilder ret =
              outer.new PartBuilder(new MyBlockStateProvider.ConfiguredModelList(models));
          outer.addPart(ret);
          return ret;
        },
        ImmutableList.of());
  ...
  /**
   * A builder for {@link MyConfiguredModel}s, which can contain a callback for processing the
   * finished result. If no callback is available (e.g. in the case of {@link
   * MyConfiguredModel#builder()}), some methods will not be available.
   *
   * <p>Multiple models can be configured at once through the use of {@link #nextModel()}.
   *
   * @param <T> the type of the owning builder, which supplied the callback, and will be returned
   *     upon completion.
   */
  public static class Builder<T> {

    private ModelFile model;
    @Nullable private final Function<MyConfiguredModel[], T> callback;
    ...
    Builder(
        @Nullable Function<MyConfiguredModel[], T> callback, List<MyConfiguredModel> otherModels) {
     ...
     * @return this builder
     * @throws IllegalArgumentException if {@code value} is not a valid x-rotation (see {@link
     *     BlockModelRotation})
     */
     ...
     * @return this builder
     * @throws IllegalArgumentException if {@code value} is not a valid y-rotation (see {@link
     *     BlockModelRotation})
     */
    ...
    /**
     * Build the most recent model, as if {@link #nextModel()} was never called. Useful for
    ...
    /**
     * Apply the contained callback and return the owning builder object. What the callback does is
     * not defined by this class, but most likely it adds the built models to the current variant
     * being configured.
     *
     * <p>Known callbacks include:
     *
     * <ul>
     *   <li>{@link VariantBlockStateBuilder.PartialBlockstate#modelForState()}
     *   <li>{@link MultiPartBlockStateBuilder#part()}
     * </ul>
     *
    ...
    public T addModel() {
      Preconditions.checkNotNull(
          callback, "Cannot use addModel() without an owning builder present");
      return callback.apply(build());
    }

    /**
     * Complete the current model and return a new builder instance with the same callback, and
     * storing all previously built models.
     *
