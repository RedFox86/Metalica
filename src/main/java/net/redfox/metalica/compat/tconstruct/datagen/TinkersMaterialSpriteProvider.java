...
public class TinkersMaterialSpriteProvider extends AbstractMaterialSpriteProvider {
  ...
  // Let's be honest. This code is shit. But it works. And the #1 rule of programming - if it
  ...
  public static Color[] getColorPixels(String name) {
    ...
    try {
      BufferedImage color =
          ImageIO.read(
              TinkersMaterialSpriteProvider.class.getResourceAsStream(
                  "/assets/" + Metalica.MOD_ID + "/textures/color/" + name + ".png"));
      for (int i = 0; i < 8; i++) {
  ...
  protected void addAllMaterials() {
    for (MetalMaterial material : MetalMaterial.getMaterials()) {
      ...
      buildMaterial(
          ...
          .statType(
              new MaterialStatsId(ResourceLocation.fromNamespaceAndPath("tconstruct", "limb")))
          .colorMapper(
